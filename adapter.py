...
class GroupAtGuardMiddleware(InboundMiddleware):
            ...
            if media_refs:
                for ref in media_refs:
                    ...
                    if kind not in ("image", "file") or not url:
                        continue
                    # Extract resourceId from URL (e.g. hunyuan.tencent.com/api/resource/download?resourceId=xxx)
                    resource_id = ""
                    try:
                        parsed = urllib.parse.urlparse(url)
                        query = urllib.parse.parse_qs(parsed.query)
                        rids = query.get("resourceId") or query.get("resourceid") or []
                        if rids:
                            resource_id = str(rids[0]).strip()
                    except Exception:
                        pass
    ...
    async def handle(self, ctx: InboundContext, next_fn) -> None:
        ...
        if ctx.chat_type == "group" and not ctx.owner_command and not self._is_at_bot(ctx.msg_body, adapter._bot_id):
            ...
            logger.info(
                "[%s] Group message observed (no @bot): chat=%s from=%s media_refs=%d",
                adapter.name, ctx.chat_id, ctx.from_account,
                len(ctx.media_refs) if ctx.media_refs else 0,
            )
